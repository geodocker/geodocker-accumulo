...
-e CCUMULO_ZOOKEEPERS=$YARN_RM \
-e ACCUMULO_SECRET=$ACCUMULO_SECRET \
-e ACCUMULO_PASSWORD=$ACCUMULO_PASSWORD \
-e INSTANCE_NAME=$INSTANCE_NAME"

if is_master ; then
    docker run -d --net=host --name=accumulo-master $DOCKER_ENV $IMAGE master
    docker run -d --net=host --name=accumulo-monitor $DOCKER_ENV $IMAGE monitor
    docker run -d --net=host --name=accumulo-tracer $DOCKER_ENV $IMAGE tracer
    docker run -d --net=host --name=accumulo-gc $DOCKER_ENV $IMAGE gc
else # is worker
